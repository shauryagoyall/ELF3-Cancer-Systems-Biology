...
def plot_heatmap(df,network_name,path_to_plots, replicate):
	...
	plt.savefig(path_to_plots+network_name+"_heatmap.png",dpi=800)
	plt.close()
...
def plot_UMAP_scatter(df, plot_path, replicate):
    ...
    UMAP.plot.scatter(x = 'UMAP_1',y='UMAP_2',s=1,c=(sub_dataframe['ZEB'] - sub_dataframe['MIR']),cmap=plt.cm.nipy_spectral)
...
def read_scoring(plot_path,phenotype):
	...
	#print(phenotype,mean_of_the_columns, sdev_of_the_columns)
...
# This function generates scatter plot of EM score vs PD-L1 expression
def plot_EM_TamRes_tri_scatter(df, plot_path, replicate, network, core_plot):
	...
	if network != "core":
		mean_PD, std_PD = read_scoring(core_plot,'TamRes')
	else:
		mean_PD, std_PD = read_scoring(plot_path,'TamRes')
	#print(phenotypes_boundary(mean_PD,std_PD)) 
    
	df['EM_score'] = (df['ZEB'] - df['MIR'])
	df["TamRes_score"] = df["ERA36"] - df["ERA66"]
	plt.scatter(df['EM_score'], df['TamRes_score'],marker="o",s=0.1,c='black')
	plt.axvline(x=boundaries[0], color='r')
	plt.axvline(x=boundaries[1], color='r')
	plt.axhline(y=-0.25, color='r')
	#plt.axhline(y=boundaries_PD[1], color='r')
	plt.xlabel('EMT score')
	plt.ylabel('TamRes Score')
	plt.tight_layout()
	plt.savefig(plot_path + "EM_TamRes_scatter.png" , dpi=500)
	plt.close()
	print(replicate + " EM-TamRes Scatter done")
...
def all_analysis(replicate,network_name):
	...
	#plot_EM_TamRes_tri_scatter(df, plot_path,  replicate, network_name, core_plot)
	...
	#plot_prob_state_tri(df, plot_path, replicate, network, core_plot)
	#plot_TamRes_PD_tri_scatter(df, plot_path, replicate, network, core_plot)
	#plot_prob_resistance_tri(df, plot_path, replicate, network, core_plot)
